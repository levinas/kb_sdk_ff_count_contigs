package ff_count_contigs::ff_count_contigsClient;
...
ff_count_contigs::ff_count_contigsClient
...
sub new
{
    ...
    my $self = {
	client => ff_count_contigs::ff_count_contigsClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a ff_count_contigs.workspace_name
$contigset_id is a ff_count_contigs.contigset_id
$return is a ff_count_contigs.CountContigsResults
...
# Authentication: required
	...
	method => "ff_count_contigs.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "ff_count_contigs.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for ff_count_contigs::ff_count_contigsClient\n";
    }
    if ($sMajor == 0) {
        warn "ff_count_contigs::ff_count_contigsClient version is $svr_version. API subject to change.\n";
    }
}
...
package ff_count_contigs::ff_count_contigsClient::RpcClient;
